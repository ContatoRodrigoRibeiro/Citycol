...
if (!function_exists('nectar_archive_gravatar_markup')) {
	...
	{
		if (
			is_author() && isset(NectarThemeManager::$options['blog_archive_author_gravatar']) &&
...
if (!function_exists('nectar_archive_header_class_output')) {
	...
	{
		if (
			(is_category() || is_tag() || is_date() || is_author()) && isset(NectarThemeManager::$options['blog_archive_bg_functionality']) &&
...
if (!function_exists('nectar_archive_header_in_title_markup')) {
	function nectar_archive_header_in_title_markup($text)
	{
		...
		if (
			isset(NectarThemeManager::$options['blog_archive_format']) &&
...
if (!function_exists('nectar_next_post_display')) {
	...
	function nectar_next_post_display()
	{
		...
		if (
			!empty($next_post) && $blog_next_post_bool ||
			...
			if ($post_pagination_style == 'fullwidth_next_prev' || $post_pagination_style == 'contained_next_prev') {
				...
				if (!empty($previous_post)) {
					...
					echo '<a href="' . esc_url(get_permalink($previous_post_id)) . '" aria-label="' . esc_attr($previous_post->post_title) . '"></a><h3><span>' . esc_html__('Post anterior', 'salient') . '</span><span class="text">' . wp_kses_post($previous_post->post_title) . '
				...
				if (!empty($next_post)) {
					...
					echo '<a href="' . esc_url(get_permalink($next_post_id)) . '" aria-label="' . esc_attr($next_post->post_title) . '"></a><h3><span>' . esc_html__('Próximo Post', 'salient') . '</span><span class="text">' . wp_kses_post($next_post->post_title) . '
						 <svg class="next-arrow" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 39 12"><line class="top" x1="23" y1="-0.5" x2="29.5" y2="6.5" stroke="#ffffff;"></line><line class="bottom" x1="23" y1="12.5" x2="29.5" y2="5.5" stroke="#ffffff;"></line></svg><span class="line"></span></span></h3>';
				...
				<div class="col span_12 dark left<?php echo esc_html($hidden_class); ?>">
					<div class="inner">
						...
						if ($blog_next_post_link_order === 'reverse') {
							echo '<span><i class="' . esc_attr($next_prev_title_class) . '">' . esc_html__('Post Anterior', 'salient') . '</i></span>';
						} else {
							echo '<span><i class="' . esc_attr($next_prev_title_class) . '">' . esc_html__('Proximo post', 'salient') . '</i></span>';
						}
					...
					while ($related_posts_query->have_posts()):
						...
						<div class="col <?php echo esc_attr($span_num); ?>">
							<div <?php post_class('inner-wrap'); ?>>
								...
								<a class="entire-meta-link" href="<?php the_permalink(); ?>"><span
										class="screen-reader-text"><?php echo get_the_title(); ?></span></a>
								...
								<?php if ($related_post_style != 'material') { ?>
									...
									<div class="post-meta">
										<span class="meta-author"> <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"> <i
													class="icon-default-style icon-salient-m-user"></i> <?php the_author(); ?></a> </span>
